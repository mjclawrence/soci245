---
title: "Stanford Open Policing Project Tutorial"
output:
...
**Note to class:** This tutorial is provided by the Open Policing Project. I have made some minor edits and added comments in sections that begin with the **Note to class:** tag (like the start of this paragraph).

## Setup

First, let's load the necessary libraries and data. 

**Note to class:** The data file is quite big, so the following chunk will take some time to finish You'll know the data loaded correctly when you have two dataframes in the global environment (in the top right pane).
...
**Note to class:** Some of this repeats the basics of R you already have or saw in last week's exercise. I'm leaving it here in case you want a refresher. 
...
Let's take a look at what columns exist in our `stops` dataframe. 
...
**Note to class:** Remember the console is the bottom left pane. You don't use fenced code blocks for code down there and there is no green "Run Current Chunk" arrow, so just type the code and press return.
...
**Note to class:** The paragraph above correctly states that `mutate()` "adds new columns to a dataframe." A potentially easier way to understand the function is that it *creates new variables*.
...
**Note to class:** The description below is a little confusing. All you really need to know is that `left_join()` is a function to merge two adta frames. 
...
**Note to class:** Do you see how they reach this conclusion from the figure? All the points would be on the dotted line if the white hit rates and the minority hit rates were the same in each district. Since most of the points fall below the line, that shows that minority hit rates are lower. 
...
**Note to class:** This next section is super interesting, but also very advanced statistically and computationally intensive. Running some of the code uses more memory than this cloud version of R Studio allows. I provide a link with the cleaned up data so you can complete the analyses, but am leaving all the code in this notebook in case you want to see how it is created. The original code is in the lines beginning with the `<!--` symbols. To free up some memory before continuing, let's delete all our current data with this helpful code:
...
**Note to class:** Run this next chunk to load the data that the previous chunks would have generated:
...
Furthermore, while this stat gives us some intuition that racial profiling might be present in Philly, we've only looked at one time. In order to make sure the result is robust, we want to check this for _every_ time in our inter-twilight period. To do this, we can use _logistic regression_. The basic idea here is that we're asking what our data implied about how darkness and clock time influence whether a stopped driver will be black. We then extract the coefficient of darkness. (You can think of this loosely as the measure of how much darkness---as opposed to clock time---influences whether a stopped driver will be black.)
...
The fact that the standard error is small means that this is a _statistically significant_ finding. 

**Note to class:** The key thing to notice about the standard error is that it is less than .05. That is why it is statistically significant (at the 95% confidence level).

Another important thing to do, besides checking statistical significance of your results, is to do a bunch of different robustness checks. The idea is that all models make some choices about how to turn the question at hand into math, and we want to vary those choices to make sure that none of them change the model result---that the result holds regardless of, for example, how you choose to control for time. In our model we used a natural spline with six degrees of freedom, but we could have chosen to use 3 degrees of freedom, or to do something way simpler like bin time to one-minute or five-minute windows.
